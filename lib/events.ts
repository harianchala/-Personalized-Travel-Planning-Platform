...
export async function getEventsData(city: string): Promise<Event[]> {
  try {
    // Try Ticketmaster API first (more reliable)
    const ticketmasterKey = process.env.NEXT_PUBLIC_TICKETMASTER_API_KEY
    if (ticketmasterKey) {
      const ticketmasterEvents = await getTicketmasterEvents(city, ticketmasterKey)
      if (ticketmasterEvents.length > 0) {
        return ticketmasterEvents
      }
    }

    console.warn("No API keys found or no events returned, using mock data")
